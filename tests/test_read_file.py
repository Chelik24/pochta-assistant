...
def test_формула_без_значения_показывается_как_формула(tmp_path):
    """Файл, собранный программой, а не Excel: значение ещё не посчитано."""
    путь = сделать_xlsx(
        tmp_path / "форма.xlsx",
        '<row r="1"><c r="A1" t="s"><v>0</v></c>'
        '<c r="B1"><f>D1*E1</f></c></row>',
        shared=("Позиция",),
    )

    assert read_file.read_any(путь) == "Позиция | =D1*E1"


def test_посчитанная_формула_показывает_число(tmp_path):
    """Файл из Excel: рядом с формулой лежит её значение — берём его."""
    путь = сделать_xlsx(
        tmp_path / "кп.xlsx",
        '<row r="1"><c r="A1" t="s"><v>0</v></c>'
        '<c r="B1"><f>D1*E1</f><v>132240</v></c></row>',
        shared=("Позиция",),
    )

    assert read_file.read_any(путь) == "Позиция | 132240"
